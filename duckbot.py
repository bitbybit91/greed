...
def _build_duckbot(cfg: nuconfig.NuConfig, token: str):
    """Internal helper that constructs a DuckBot class wired to *token* with
    error-handling behaviour driven by *cfg*."""
...
def factory(cfg: nuconfig.NuConfig):
    ...
    return _build_duckbot(cfg, cfg["Telegram"]["token"])


def factory_with_token(cfg: nuconfig.NuConfig, token: str):
    """Construct a DuckBot type based on the passed config, using a specific bot token."""
    return _build_duckbot(cfg, token)
