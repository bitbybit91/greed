...
def factory_with_token(cfg: nuconfig.NuConfig, token: str):
    """Construct a DuckBot type using an explicit token instead of the one in config.

    Behaves identically to :func:`factory` but uses the provided token for
    the underlying :class:`telegram.Bot` instance. All retry and error-handling
    logic is identical.
    """
