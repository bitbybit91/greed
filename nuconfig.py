...
# Top-level sections that are optional (may be present in template but absent in user config)
...
class NuConfig:
    ...
    def __compare_recurse(self: dict, other: dict, top_level: bool = False) -> CompareReport:
        ...
        for key, value in self.items():
            # Skip optional top-level sections that may not be present in the user config
